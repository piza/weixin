 ...
 * @since 1.2
 */
public final class ApiConfigMemeory extends Observable implements ApiConfig {

    private static final Logger logger = LoggerFactory.getLogger(ApiConfigMemeory.class);
    /**
     ...
     */
    public ApiConfigMemeory(String appid, String secret) {
     ...
     */
    public ApiConfigMemeory(String appid, String secret, boolean enableJsApi) {
        this.appid = appid;
        this.secret = secret;
        ...
        try {
            ...
            if (time > 7100000 && this.tokenRefreshing.compareAndSet(false, true)) {
                if (logger.isWarnEnabled()) {
                    logger.warn("准备刷新token.............appId is "+appid);
                }
                initToken(now);
            }
        } catch (Exception e) {
    ...
    private void initToken(final long refreshTime) {
        ...
        final String url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=" + this.appid + "&secret=" + this.secret;
